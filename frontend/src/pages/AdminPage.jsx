...
import { getPendingUsers, approveUser, rejectUser, apiFetch } from '../services/api';
import { CheckCircle, XCircle, ShieldAlert, LogOut, Briefcase, Activity, Users } from 'lucide-react';
...
const AdminPage = () => {
  ...
  const handleReject = async (userId, username) => {
    if (!window.confirm(t('admin.approval.confirmReject', { username }))) return;

    try {
      await rejectUser(userId);
      setMessage(t('admin.approval.successReject', { username }));
      fetchUsers(); 
      setTimeout(() => setMessage(''), 3000);
    } catch (err) {
      alert(t('admin.approval.fail') + err.message);
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          ...
          <button onClick={handleLogout} className="auth-button" style={{ background: '#ef4444', border: 'none' }}>
            <LogOut size={16} /> {t('header.nav.logout')}
          </button>
        </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                {pendingUsers.map(user => (
                  <div key={user.id} style={{
                    ...
                    {/* BAGIAN TOMBOL: Ditambahkan gap: '1rem' */}
                    <div style={{ display: 'flex', justifyContent: 'flex-end', borderTop: '1px solid #e2e8f0', paddingTop: '1rem', gap: '1rem' }}>
                      ...
                      >
                        <XCircle size={18} /> {t('admin.approval.reject')}
                      </button>
